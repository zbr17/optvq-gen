...
def get_optvq_tokenizer(config):
    from optvq.models.vqgan_hf import VQModelHF
    name = config.model.vq_model.name
    tokenizer = VQModelHF.from_pretrained(name)
    tokenizer.eval()
...
def main():
    ...
    logger = setup_logger(name="OptVQ-Gen", log_level="INFO",
    ...
    tokenizer = get_optvq_tokenizer(config)
